    ...
    <div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 mb-6 flex items-center gap-6">
        <div className="relative">
          <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden border">
            {user?.user_metadata?.avatar_url || user?.avatar_url ? (
              <img src={user?.user_metadata?.avatar_url || user?.avatar_url} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
          ...
          <button 
            ...
            disabled={uploading}
            className="absolute bottom-0 right-0 p-2 bg-orange-500 text-white rounded-full hover:bg-orange-600 disabled:opacity-50"
          >
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Cập nhật ảnh đại diện để cá nhân hóa tài khoản của bạn</p>
          {uploading && <p className="text-orange-500 text-sm mt-1">Đang tải ảnh lên...</p>}
        </div>
      </div>
        ...
        {showPasswordForm && (
          <form onSubmit={handlePasswordChange} className="space-y-4">
            ...
            <div className="flex gap-4">
              <button type="submit" disabled={changingPassword} className="bg-orange-500 text-white px-8 py-2 rounded-lg hover:bg-orange-600 transition-colors">
